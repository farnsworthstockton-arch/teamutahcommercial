...
- [ ] ⚠️ 1145 Washington Blvd Ogden OM — the CAP RATE on page 3 was not updated with the price. It still reads "net annual income $105,136 with a cap rate of 6.22%", which is $105,136 / $1,690,000 (the OLD price). At the new $1,550,000 the same income is a 6.78% cap. The OM now shows a $1,550,000 price next to a 6.22% cap — those two numbers cannot both be true, and 6.78% is the better selling number. Needs a re-export from RealNex. Same fix needed on Crexi (property 2632420) and anywhere else the OM or the cap rate is posted.
